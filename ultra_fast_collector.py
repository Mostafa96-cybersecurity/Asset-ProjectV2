...
def _collect_windows_standalone(ip: str, username: str, password: str) -> Optional[Dict]:
    ...
    # Suppress Win32 COM warnings (these are harmless cleanup notifications)
    import warnings
    warnings.filterwarnings("ignore", message=".*Win32 exception occurred releasing.*", category=UserWarning)
    
    try:
        if not COLLECTION_UTILS_AVAILABLE:
            return None
        
        # Initialize COM for WMI with proper error handling
        ...
        try:
            ...
            # If pythoncom is not available, try alternative initialization
            try:
        ...
        # WMI connection with timeout and proper error handling
        ...
        try:
            # Enhanced Physical Disk Information (WORKING VERSION)
            # Based on demonstrate_drive_counting.py success
            ...
            # Collect physical drives info (hardware level)
            ...
            # Collect free space from logical drives (for free space calculation only)
            try:
                logical_drives = conn.Win32_LogicalDisk()
                total_free_space = 0
                
                for drive in logical_drives:
                    if drive.FreeSpace:
                        total_free_space += int(drive.FreeSpace)
                
                data['free_disk_space'] = str(total_free_space)
            except:
                pass
        except:
            pass
        ...
        # Explicit cleanup of WMI connection and objects
        try:
            # Clear the connection to free COM objects
            conn = None
            import gc
            gc.collect()  # Force garbage collection before COM cleanup
        except:
            pass
        
        # Cleanup COM properly if we initialized it
        if com_initialized:
            try:
                import pythoncom
                pythoncom.CoUninitialize()  # type: ignore
        ...
        # Explicit cleanup of WMI connection and objects on error
        try:
            # Clear the connection to free COM objects
            if 'conn' in locals():
                conn = None
        ...
        # Cleanup COM on error if we initialized it
...
def _enhanced_device_collection(ip: str, os_info: Dict[str, str], win_creds: Optional[List] = None, linux_creds: Optional[List] = None, snmp_communities: Optional[List] = None) -> Optional[Dict]:
    """
    Enhanced device collection using hierarchical strategy based on OS detection
    ...
    """
    # Initialize parameters with empty lists if None
    win_creds = win_creds or []
    linux_creds = linux_creds or []
    snmp_communities = snmp_communities or []
    
    os_family = os_info.get('os_family', 'Unknown')
    confidence = int(os_info.get('confidence', '0'))
    
    log.info(f"🎯 Enhanced collection for {ip}: {os_family} ({confidence}% confidence)")
    
    # Initialize credentials
    win_creds = win_creds or [{'username': '', 'password': ''}]  # Current user
    linux_creds = linux_creds or [{'username': 'root', 'password': ''}]
    snmp_communities = snmp_communities or ['public', 'private']
    
    collection_data = {
        'ip_address': ip,
        'detection_method': os_info.get('detection_method', 'Port-based'),
        'os_detection_confidence': confidence,
    ...
    if os_family == 'Windows':
        ...
        for i, creds in enumerate(win_creds):
            try:
                log.debug(f"      Trying WMI credentials {i+1}/{len(win_creds)}")
                data = _collect_windows_standalone(ip, creds.get('username', ''), creds.get('password', ''))
    ...
    # Linux Strategy: SSH → SNMP
        ...
        for i, creds in enumerate(linux_creds):
            try:
                log.debug(f"      Trying SSH credentials {i+1}/{len(linux_creds)}")
    ...
    # Other Devices Strategy: SNMP → SSH
        ...
        # HTTP fallback for web devices
        http_data = _collect_http_standalone(ip)
        if http_data:
    ...
    collection_data.update({
        'collection_strategy': 'Hierarchical (All Methods Failed)',
        'wmi_collection_status': 'Failed: All methods exhausted',
        'error': f'All collection methods failed for {os_family} device'
    })
    
    log.warning(f"   ❌ All collection methods failed for {ip}")
    return collection_data

def _try_snmp_collection(ip: str, communities: List[str]) -> Optional[Dict]:
    """Helper function to try SNMP with multiple communities"""
    for community in communities:
        try:
            log.debug(f"      Trying SNMP community: {community}")
            data = _collect_snmp_standalone(ip, [community])
            if data:
                data['snmp_community'] = community
                return data
        except Exception as e:
...
def _port_based_os_detection(ip: str) -> Dict[str, str]:
    ...
    result = {'os_family': 'Unknown', 'device_type': 'Unknown', 'confidence': '0', 'detection_method': 'Port-based'}
